...
// (Clerk mock). The Anon variant (sessionId:null, signed-out) takes the ephemeral
// sessionStorage path with NO network — the primary crash-free proof. The SignedIn
// variant fires a mount-time fetch to /api/chat-with-lens/history; there's no
// backend in Storybook so it rejects, but the component guards with `r.ok`, so it
// falls back to an empty thread rather than crashing.
...
// Anonymous, unsaved thread — no network on mount.
